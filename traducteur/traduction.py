...
def file_to_panda(filePath): # desuet
...
def compendium_to_panda(filePath): # desuet
    ...
    for i, row in resized_transcript_info.iterrows():
        ...
        if (motif_start in range(cDNA_start, cDNA_end+1)) and (motif_end in range(cDNA_start, cDNA_end+1)):
            ...
            if strand == 1:     # translation for positive strands
                ...
                start_translation = np.append(start_translation, (genome_start + motif_start - cDNA_start))
                end_translation = np.append(end_translation, (genome_start + motif_end - cDNA_start))

            elif strand == -1:  # translation for negative strands

                end_translation = np.append(end_translation, (genome_end - motif_start + cDNA_start))
                start_translation = np.append(start_translation, (genome_end - motif_end + cDNA_start))
...
def translation(dataset, transcript):
    ...
    for k, row in dataset.iterrows():
        ...
        if two_lines:
            # increase array sizes to match final length with exon spliced motifs
...
def rbp_classify():
    ...
    for file in os.listdir(output_fruitfly):

        if file.endswith(".tsv"):

            input_file = open(output_fruitfly + file, "r")

            for line in input_file:

                rbp = line.split()[4]
                output_file = open(rbp_classified_dir_fruitfly + rbp + ".tsv", "a")

                output_file.write(line[:-(len(rbp)+1)] + "\n")

            print(file)

    for file in os.listdir(output_human):

        if file.endswith(".tsv"):

            input_file = open(output_human + file, "r")

            for line in input_file:

                rbp = line.split()[4]
                output_file = open(rbp_classified_dir_human + rbp + ".tsv", "a")
...
def browser_header():
    ...
    for motif in compendium["MOTIFS_ID"]:

        current_file = open("../rbp_classified/fruitfly/" + motif + ".tsv", "w")
        current_file.write("track type=bedGraph name=" + motif + " color=0,0,0 visibility=full\n")

    for motif in compendium["MOTIFS_ID"]:

        current_file = open("../rbp_classified/human/" + motif + ".tsv", "w")
        current_file.write("track type=bedGraph name=" + motif + " color=0,0,0 visibility=full\n")
